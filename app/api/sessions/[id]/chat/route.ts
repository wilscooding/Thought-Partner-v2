...
export async function POST(
    ...
    try {
        ...
        if (!session.avatar?.systemPrompt) {
            return NextResponse.json({ error: "Avatar systemPrompt is missing" }, { status: 500 });
        }
        ...
        // 3) Fetch history - ADDED TYPE ASSERTION HERE
        const historyData = await prisma.message.findMany({
            where: { sessionId },
            orderBy: { createdAt: "desc" },
            take: maxHistory,
            select: { role: true, content: true },
        }) as ChatMessage[]; // This tells TS: "This array follows the ChatMessage shape"
